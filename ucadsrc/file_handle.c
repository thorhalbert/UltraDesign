...
BOOL descend;
{
	...
	if (descend) {
		...
		if ((a=strlen(filepath))==0)
			newbuf[0] = '\0';
		else if (strchr(current_in->FileName,':')!=NULL)
...
UBYTE *outform;
{
	...
	if (((int) current_in)==1) {
		strcpy(outform,"/ Parent");
		return;
	}

	if (((int) current_in)==2) {
		strcpy(outform," [List Devices]");
		return;
	}

	st = current_in->FileName;
	while (*st==' ') st++;
...
UBYTE *instr;
{
	...
	c1 = GI_DoAlloc(strlen(instr)+1,userem);
...
struct File_Entry *instr;
{
	...
	inent = (struct File_Entry *) 
		GI_DoAlloc(sizeof(struct File_Entry),userem);
...
filereq_setup()
{
	finfo= (struct FileInfoBlock *) 
			GI_RemAlloc(sizeof (struct FileInfoBlock));
...
VOID ClearGet()
{
	...
	if (*userem!=NULL) GI_DoFree(userem);

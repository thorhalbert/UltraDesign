 ...
 *	3	19-May-90 CK	Rewrite English Architectural and
 *				English Feet/Inches (and good riddance!-SH)
 *
 */
...
STDFLT x;
{
	...
	STDFLT a,b,c,d,e,f,itol;
	int j,k,l,n,m;
	UBYTE *out,test[50],frc[20],frc2[20],*sgn;
	BOOL neg;
	...
	if (linpoints>8) linpoints = 8;

	sgn = neg?"-":"";
	switch (lintype) {
	case LIN_EDI:

		d = 1.0;			/* calc 10^linpoints */
		n = linpoints;
		while (n >= 1) {
			d *= 10.0;
			n--;
		}
		c = 1.0/d;			/* (1.0, 0.1, 0.01, ...) */
		a = x * MM_IN;			/* Cnvt from mm to in */
		j = a / 12;			/* Number of ft */
		b = a - j*12;			/* floating inches */
		
		b += 0.5 * c;		/* Rounding to */
		b  = floor(b/c) * c;	/* Cth place   */

		if (b >= 12.0) {		/* If inches rounded up */
			b = 0.0;		/* reset inch and     */
			j++;			/* increment feet   */
		}
		
		if (j == 0 && b == 0.0)		/* In case tiny neg number */
			neg = FALSE;		/* was rounded to zero	   */

		sprintf(frc,"%%.%df",linpoints);	/* %.nf mask */
		sprintf(frc2,frc,b);
	...
	case LIN_EFI:
		...
		a = x * MM_IN;			/* Cnvt from mm to in */
		j = a / 12.0;			/* Number of ft */
		k = a - j*12;			/* Whole inches */
		b = a - floor(a);		/* Decimal frac of inches */
		n = (b + 0.5/itol) * itol;	/* Numerator */
		m = itol;			/* Denominator */
		
/* CHECK IF THIS NEXT LINE TESTS DIVISIBILITY OF n BY 2 PROPERLY: */

		while ((n&1)==0 && n>1) {	/* Reduce fraction */
			n >>= 1;
			m >>= 1;
		}
		
		if (m==1 && n==1) {	/* If frac rounded up */
			n = 0;		/* reset frac and     */
			k++;		/* increment inches   */
		}
		
		if (k >= 12) {			/* If inches rounded to 12 */
			k = 0;			/* reset inches and	   */
			j++;			/* increment feet	   */
		}
		
		if (j == 0 && k == 0 && n == 0)	/* In case tiny neg number */
			neg = FALSE;	/* was rounded to zero	   */

		if (n<1) frc[0]='\0';
		else sprintf(frc," %d/%d",n,m);

		sprintf(test,"%s%d'%d%s\"",sgn,j,k,frc);
		...
		if (n>=(m>>1)) k++;	/* Round the fracture UP dude */

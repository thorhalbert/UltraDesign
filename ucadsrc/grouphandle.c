 ...
 *	2	 6-May-90 SH	Link_Group should not always clear descendants
 *
 */
...
setpath()
{
	...
	if (streq(odis,"     ")) strcpy(odis,"None ");

	sprintf(GroupTitle,"Group Name       Count: %4d   Flags: %s",
		Current_Group->Use_Count,odis);
...
GROUP_A *Link_Group(inent,inmother, cleardesc)
...
GROUP_A *groupinsert (instr,inmother, cleardesc)
...
BOOL cleardesc;
{
	...
	*inent = instr;
	...
	Link_Group(inent,inmother,cleardesc);

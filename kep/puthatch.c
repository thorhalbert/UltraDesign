...
GROUP_A *ingroup;
{
	...
	if (gotbounds!=4) {
		complain("Continue Next",NULL,"Hatch table pattern",
					 ingroup->Group_Name,
...
GROUP_A *ingroup;
{
	...
	bboxmaxy = bboxmaxx = -HUGE_VAL;	/* Prep max min fun */
	bboxminy = bboxminx = HUGE_VAL;
...
GROUP_A *ingroup;
{
	...
	lastxelement = lastyelement = HUGE_VAL;
	population = 0;
...
BOOL TraverseGroups()
{
	...
	obuf1.Hatch_Magic = CUR_HATCH_VER;
	obuf1.Hatch_Version = HAT_MAGIC_NUM;
	...
	if (looper==NULL) {
		complain("Recover",NULL,"The group structure in your",
	...
	if (!anythinghappen) {	
		complain("Recover",NULL,"I couldn't find anything to",
